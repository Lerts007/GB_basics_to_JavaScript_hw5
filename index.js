...
const sale15 = (num) => num - num * (15 / 100);

products.forEach((el) => {
  el.price = sale15(el.price);
  console.log(el);
});
...
const productsNewInPhoto = products1.filter((el) => {
...
console.log(productsNewInPhoto);

const productSortMin = products1.sort((a, b) => a.price - b.price);
console.log("Отсортировать массив, начиная с самой маленькой цены:");
console.log(productSortMin);
...
const en = ["mon", "tue", "wed", "thu", "fri", "sat", "sun", "Asd"];
const ru = ["понедельник", "вторник", "среда", "четверг", "пятница", "суббота", "воскресенье"];
const dayEnRu = {};

for (let i = 0; i < en.length; i++) {
  // Делаю проверку, чтобы значений оказалось не больше чем ключей
  if (ru[i] !== undefined) {
    dayEnRu[en[i]] = ru[i];
  }
}
